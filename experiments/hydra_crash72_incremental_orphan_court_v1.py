...
def main(csv,out):
    d=pd.read_csv(csv).copy(); d['date']=pd.to_datetime(d['open_time'],utc=True); d=d.sort_values('date').reset_index(drop=True)
    ...
    # Reconstruct the three frozen classes. Exceptions are native positives not in base.
    ex=(y0==1)&(~base)
    trans=ex & d['entry_path'].notna() & d['entry_path'].astype(str).ne('nan')
    orphan=ex & ~trans
    core=y0.astype(bool)&base
    ...
    # Strict prior-year train -> current-year holdout; no random split.
    rows=[]
    for year in sorted(d.date.dt.year.unique()):
        ...
        for target in ['target_core','target_transition','target_orphan']:
            ...
            for name,col in feature_map.items():
                xtr=train[[col]].apply(pd.to_numeric,errors='coerce').fillna(train[col].median()).values; xt=test[[col]].apply(pd.to_numeric,errors='coerce').fillna(train[col].median()).values
                m=LogisticRegression(max_iter=2000).fit(xtr,ytr); p=m.predict_proba(xt)[:,1]
                rows.append({'year':year,'target':target,'model':name,**metrics(yt,p)})
            # nested Hazard -> Hazard+RR incremental test
            cols=['hazard_score','rr_recovery_minus_burden']; xtr=train[cols].apply(pd.to_numeric,errors='coerce').fillna(train[cols].median()); xt=test[cols].apply(pd.to_numeric,errors='coerce').fillna(train[cols].median())
            m=LogisticRegression(max_iter=2000).fit(xtr,ytr); p=m.predict_proba(xt)[:,1]
            rows.append({'year':year,'target':target,'model':'Hazard+RR',**metrics(yt,p)})
    result={'experiment':'hydra_crash72_incremental_orphan_court_v1','rows':rows,'class_counts':{k:int(d[k].sum()) for k in ['target_core','target_transition','target_orphan']}}
