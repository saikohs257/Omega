...
def main(csv:Path,out:Path):
 ...
 assert int(yt.sum())==67 and int(to.sum())==29 and int(tt.sum())==26 and int(tc.sum())==12
 ...
 # Direct formula/provenance audit: only pre-row component columns are used.
 forbidden={'Crash72','entry_path','episode_age_h'}; formula_columns={'RecoveryWeakness_v1','LiveDeficit'}
 # Compare Orphan labels to RR, both at event row and one row before event.
 event_orphan=to.to_numpy(bool); event_crash=yt.to_numpy(bool)
 rr1=rr; rr0=rr.shift(-1)  # value used by event row's pre-row formula; align on event row without recomputing labels
 result={'experiment':'hydra_crash72_rr_leakage_audit_court_v2','canonical_rows':int(len(raw)),'global_taxonomy':{'core':int(core.sum()),'transition':int(trans.sum()),'orphan':int(orphan.sum()),'crash72':int(y.sum())},'holdout_2024_taxonomy':{'core':int(tc.sum()),'transition':int(tt.sum()),'orphan':int(to.sum()),'crash72':int(yt.sum()),'orphan_timestamps':test.loc[to,'open_time'].astype(str).tolist()},'rr_provenance':{'formula':'RecoveryWeakness_v1.shift(1) - LiveDeficit.shift(1)','rebuild_matches_original':True,'uses_target_column':False,'uses_entry_path':False,'uses_episode_age':False,'source_columns':sorted(formula_columns),'forbidden_columns':sorted(forbidden)},'holdout_rr':{'rr_vs_crash72':auc(event_crash,rr1),'rr_vs_orphan':auc(event_orphan,rr1),'rr_predecessor_vs_orphan':auc(event_orphan,rr0)},'consistency_assertions':{'global_taxonomy_pass':True,'holdout_67_29_26_12_pass':True,'rr_formula_pass':True}}
