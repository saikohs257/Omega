...
def fit_auc(train, test, feature, target):
    ...
    if len(tr) < 200 or tr[target].nunique() < 2 or te[target].nunique() < 2:
        return None
    ...
    m.fit(tr[[feature]].astype(float), tr[target].astype(int))
    p = m.predict_proba(te[[feature]].astype(float))[:, 1]
...
def main(csv: Path, out: Path):
    raw = pd.read_csv(csv)
    native = raw.copy()
    native["open_time"] = pd.to_datetime(native["open_time"], utc=True)
    native = native.sort_values("open_time").reset_index(drop=True)
    d = history(raw)
    ...
    for k, v in targets.items():
        d[k] = v
    native_72 = pd.to_numeric(native["Crash72"], errors="coerce").fillna(0).astype(int).to_numpy()
