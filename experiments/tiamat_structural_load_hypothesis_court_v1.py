...
This is an identification experiment, not a canonical generator. It asks whether
an explicit burden-vs-recovery state explains native admission/path behavior
better than LiveDeficit alone. No target-derived feature is used.
"""
...
def active_auc(x,year,feature):
 # 24h-before-start controls: same calendar year, non-start inactive rows.
 ...
 rng=np.random.default_rng(17); c=c.iloc[rng.permutation(len(c))[:min(len(c),len(s)*20)]]
...
def main(csv,out):
 ...
 # Pairwise 2024 matched-state residual test: current state matched on canonical four;
 # compare the candidate load variable inside 168h-separated start/non-start pairs.
 q=x[(x.year==2024)&x.LiveDeficit.notna()&x.RecoveryWeakness_v1.notna()&x.hazard_raw.notna()&x.SimpleShock.notna()].copy()
 cols=['SimpleShock','LiveDeficit','RecoveryWeakness_v1','hazard_raw']; z=(q[cols]-q[cols].median())/(q[cols].quantile(.75)-q[cols].quantile(.25)).replace(0,1)
 ...
 for f in feats:
  vals=[]
  for a,b in rows:
   va,vb=float(q.iloc[a][f]),float(q.iloc[b][f])
   vals.append(1.0 if va>vb else .0 if va<vb else .5)
