...
2024 is a frozen holdout; candidate selection uses discovery years only.
Every candidate logs AUC, PR AUC, Brier, log loss, prevalence, counts, and
walk-forward metrics. AUC is explicitly undefined when a fold has one class.
"""
...
def history(d: pd.DataFrame) -> pd.DataFrame:
    ...
    # Every candidate uses only prior information at prediction time.
    ...
    d["rr_hazard_weighted_recovery"] = d.recovery_l1 / (1.0 + hs.shift(1).rolling(24, min_periods=6).mean().abs())
    return d
...
def discovery_report(d: pd.DataFrame, col: str) -> dict:
    ...
    for year in DISCOVERY:
        tr = d[d.open_time.dt.year.isin([y for y in DISCOVERY if y != year])]
    ...
    return {"folds": rows, "mean_auc": float(np.mean(valid_auc)) if valid_auc else None, "worst_auc": float(np.min(valid_auc)) if valid_auc else None, "mean_brier": float(np.mean([v["brier"] for v in rows.values()]))}
...
def main(csv: Path, out: Path) -> None:
    ...
    payload = {
        ...
        "selection_rule": "highest mean discovery AUC, tie-break lower mean discovery Brier; 2024 untouched",
