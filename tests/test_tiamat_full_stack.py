...
def test_full_dynamics_surface_is_deterministic_and_bounded() -> None:
    ...
    shock_a = simple_shock(abs_ret=0.2, rv24=0.3, range_pct=0.4, log_qv=0.5, imb_abs=0.6, medians=medians, mads=mads)
    shock_b = simple_shock(abs_ret=0.2, rv24=0.3, range_pct=0.4, log_qv=0.5, imb_abs=0.6, medians=medians, mads=mads)
...
def test_engine_evaluate_uses_real_tiamat_transition_for_state_requests() -> None:
    engine = TiamatEngine()
    decision = engine.evaluate(TiamatState().to_dict(), {"V": 0.1})
...
def test_engine_replay_matches_repeated_steps() -> None:
    engine = TiamatEngine()
    initial = TiamatState()
    evidence = ({"V": 0.1}, {"V": 0.1, "B": 0.2}, {"D": 0.9, "damage_threshold": 0.8})
    state = initial
    for row in evidence:
...
def test_diagnose_accepts_canonical_state_for_matched_damage_pokes() -> None:
    engine = TiamatEngine()
    # D is deliberately omitted from evidence: transition() treats evidence D
    # as the observed next-state D and would overwrite the state-only poke.
