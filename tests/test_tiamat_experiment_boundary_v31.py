...
from tiamat import IdentificationRunner, LockedModelEvaluator
...
from tiamat.metric_contract import ProbabilityContract, STATE_SPACE, TargetProvenance
...
def test_config_rejects_markov_lag_above_hard_boundary():
    with pytest.raises(ValueError, match=r"\[1, 10\]"):
        TournamentConfig(max_markov_lag=11)


def test_causal_gate_rejects_lookback_above_hard_boundary():
    with pytest.raises(ValueError, match=r"\[1, 10\]"):
        TemporalCausalGate(max_lookback=11)
...
def test_target_provenance_is_not_feature_provenance():
    target = TargetProvenance("observed", "mode-v1", "observed controller mode", 1.0, ("mode",), _h("a"))
    payload = target.to_dict()
    assert "target_dependencies" in payload
    assert "label_feature_dependencies" not in payload


def test_probability_output_boundary_rejects_invalid_model_output():
    contract = ProbabilityContract(STATE_SPACE)
    row = {"timestamp": "2026-08-08T10:00:00Z", "B": 0.2, "V": 0.1, "D": 0.0, "tau_D": 0.0, "tau_mode": 0.0, "mode": "Q"}

    def invalid(_row):
        return {state: (1.0 if state == "Q" else 0.0) for state in STATE_SPACE} | {"X": 0.0}

    evaluator = LockedModelEvaluator("M3", IdentificationRunner(), invalid, contract)
    with pytest.raises(ValueError, match="state-space mismatch"):
...
def test_probability_output_boundary_rejects_nan():
    ...
    def invalid(_row):
        return {state: (math.nan if state == "Q" else 0.0) for state in STATE_SPACE}
