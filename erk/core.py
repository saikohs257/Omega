...
class Authority(IntEnum):
    OBSERVE=0; SIMULATE=1; EXECUTE=2
class Action(StrEnum):
    BLOCK="BLOCK"; BRANCH="BRANCH"; REJECT="REJECT"; QUARANTINE="QUARANTINE"; ENABLE_EXECUTION="ENABLE_EXECUTION"; ESCALATE="ESCALATE"; ARCHIVE="ARCHIVE"
def _freeze(value):
...
class EvidenceRecord:
    evidence_id:str; source:str; timestamp:str; payload:Mapping[str,Any]; authority_grant:int|None=None; authority_signature:str=""; provenance_hash:str=""; authority_grant_id:str|None=None
    def __post_init__(self):
        object.__setattr__(self,"payload",_freeze(self.payload)); digest=hashlib.sha256(self._canonical_content()).hexdigest()
    ...
    def _content(self,include_signature=True): return {"evidence_id":self.evidence_id,"source":self.source,"timestamp":self.timestamp,"payload":_canonical(self.payload),"authority_grant":self.authority_grant,"authority_grant_id":self.authority_grant_id,"authority_signature":self.authority_signature if include_signature else ""}
    def _canonical_content(self): return json.dumps(self._content(),sort_keys=True,separators=(",",":"),default=str).encode("utf-8")
    def authority_message(self,state):
...
@dataclass(frozen=True,slots=True)
class GraphNode: node_id:str; kind:str; supported:bool=True
@dataclass(frozen=True,slots=True)
class GraphEdge: source:str; target:str; kind:str="DEPENDS_ON"
@dataclass(frozen=True,slots=True)
class GraphMetrics: unsupported_depth:int; critical_load:Mapping[str,int]; cycles:tuple[tuple[str,...],...]
def graph_metrics(nodes,edges):
    ids={node.node_id for node in nodes}; adjacency={node_id:[] for node_id in ids}
    for edge in edges:
        if edge.kind=="DEPENDS_ON" and edge.source in ids and edge.target in ids: adjacency[edge.source].append(edge.target)
    cycles=[]; color={node_id:0 for node_id in ids}; stack=[]
    def visit(node_id):
        color[node_id]=1; stack.append(node_id)
        for target in adjacency[node_id]:
            if color[target]==0: visit(target)
            elif color[target]==1: start=stack.index(target); cycles.append(tuple(stack[start:]+[target]))
        stack.pop(); color[node_id]=2
    for node_id in sorted(ids):
        if color[node_id]==0: visit(node_id)
    memo={}
    def depth(node_id,visiting):
    ...
    critical={}
    for node_id in sorted(ids):
        seen=set(); todo=list(adjacency[node_id])
        while todo:
            target=todo.pop()
...
def _prediction_distance(first,second):
...
def compute_strain(hypotheses,predictions,observability,relevance=None,lam=1.0):
    lam=float(lam)
    if not math.isfinite(lam) or lam<0.0: raise ValueError("lam must be finite and non-negative")
    if not hypotheses:return 0.0
    probabilities={name:_finite_nonnegative(probability,f"hypotheses[{name}]") for name,probability in hypotheses.items()}; total=sum(probabilities.values())
    if not math.isfinite(total):raise ValueError("hypotheses total must be finite")
    if total<=0:return 0.0
    probabilities={name:probability/total for name,probability in probabilities.items()}; validated_observability={variable:_unit_interval(observed,f"observability[{variable}]") for variable,observed in observability.items()}; validated_relevance={variable:_finite_nonnegative(weight,f"relevance[{variable}]") for variable,weight in (relevance or {}).items()}; names=list(probabilities); conflict=0.0
    for index,first in enumerate(names):
        for second in names[index+1:]:
            disagreement=sum(validated_relevance.get(variable,1.0)*observed*_prediction_distance(predictions.get(first,{}).get(variable),predictions.get(second,{}).get(variable)) for variable,observed in validated_observability.items() if observed>0); conflict+=probabilities[first]*probabilities[second]*disagreement
    return 1.0-math.exp(-lam*max(0.0,conflict))
@dataclass(frozen=True,slots=True)
class EpistemicState:
    observability:Mapping[str,float]=field(default_factory=dict); hypotheses:Mapping[str,float]=field(default_factory=dict); predictions:Mapping[str,Mapping[str,Any]]=field(default_factory=dict); relevance:Mapping[str,float]=field(default_factory=dict); strain:float=0.0; unsupported_depth:int=0; critical_load:Mapping[str,int]=field(default_factory=dict); cycles:tuple[tuple[str,...],...]=(); authority:Authority=Authority.OBSERVE; calibration_error:float=0.0; active_branches:int=1; evidence_count:int=0; policy_version:str="erk-v2.2"; terminal:str|None=None; used_authority_grants:tuple[str,...]=()
    def __post_init__(self):
...
@dataclass(frozen=True,slots=True)
class PolicyConfig:
    u_crit:float=.80; depth_bound:int=8; calibration_crit:float=.25; branch_bound:int=16; cost_weights:Mapping[Action,float]=field(default_factory=lambda:{Action.BLOCK:.2,Action.BRANCH:.4,Action.ARCHIVE:.6,Action.QUARANTINE:.8,Action.REJECT:1.0,Action.ESCALATE:.9,Action.ENABLE_EXECUTION:0.0})
    def __post_init__(self):
...
class Supervisor:
    def __init__(self,config=None):self.config=config or PolicyConfig()
...
class Transition:
    @staticmethod
    def apply(state,action,evidence=(),authorized_authority=None,branch_bound=16):
        state=state.normalized()
        if state.terminal is not None:raise ValueError("terminal branch cannot transition")
        branch_bound=_positive_int(branch_bound,"branch_bound"); evidence=tuple(evidence); grant_ids=tuple(record.authority_grant_id for record in evidence if record.authority_grant is not None and record.authority_grant_id is not None)
        ...
        if authorized_authority is not None and not evidence and action!=Action.ESCALATE:raise ValueError("authority escalation requires kernel authorization")
...
class SupervisorDecision:
    def __init__(self,action,rationale):self.action,self.rationale=action,rationale
