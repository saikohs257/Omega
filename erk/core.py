...
class EvidenceRecord:
    ...
    def _content(self, include_signature: bool = True) -> dict[str, Any]:
        return {"evidence_id": self.evidence_id, "source": self.source, "timestamp": self.timestamp, "payload": _canonical(self.payload), "authority_grant": self.authority_grant, "authority_signature": self.authority_signature if include_signature else ""}
...
class EpistemicState:
    ...
    def normalized(self) -> EpistemicState:
        return replace(self, observability={key: min(1.0, max(0.0, float(value))) for key, value in self.observability.items()}, strain=min(1.0, max(0.0, float(self.strain))), calibration_error=min(1.0, max(0.0, float(self.calibration_error))), active_branches=max(0, int(self.active_branches)), evidence_count=max(0, int(self.evidence_count)))
...
class Supervisor:
    ...
    def safe_actions(self, state: EpistemicState) -> tuple[Action, ...]:
        ...
        if state.authority == Authority.EXECUTE and state.strain < self.config.u_crit and state.unsupported_depth < self.config.depth_bound and state.calibration_error < self.config.calibration_crit and state.active_branches <= self.config.branch_bound: actions.append(Action.ENABLE_EXECUTION)
