...
@dataclass(frozen=True, slots=True)
class PolicyConfig:
    u_crit:float=.80; depth_bound:int=8; calibration_crit:float=.25; branch_bound:int=16; cost_weights:Mapping[Action,float]=field(default_factory=lambda:{Action.BLOCK:.20,Action.BRANCH:.40,Action.ARCHIVE:.60,Action.QUARANTINE:.80,Action.REJECT:1.0,Action.ESCALATE:.90,Action.ENABLE_EXECUTION:0.0})
    def __post_init__(self):
