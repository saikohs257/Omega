...
TOURNAMENT_CONFIG_VERSION = "tournament-config-v2"
TEMPORAL_CAUSAL_GATE_VERSION = "causal-gate-v2"
...
class TournamentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.max_markov_lag) < 1: raise ValueError("max_markov_lag must be at least 1")
        object.__setattr__(self, "max_markov_lag", int(self.max_markov_lag))
...
class TemporalCausalGate:
    ...
    def __post_init__(self) -> None:
        if int(self.max_lookback) < 1: raise ValueError("max_lookback must be at least 1")
        object.__setattr__(self, "max_lookback", int(self.max_lookback))
    ...
    def feature_horizons(self) -> dict[str, int]:
        ...
        def horizon(name: str, trail: tuple[str, ...] = ()) -> int:
            ...
            value = max([int(f.max_temporal_offset)] + [horizon(p, trail + (name,)) for p in f.derived_from])
            memo[name] = value
            return value
    ...
    def validate_row(self, row: Mapping[str, Any]) -> None:
        # Legacy name checks remain as a defense-in-depth layer; provenance is authoritative.
