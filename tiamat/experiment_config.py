...
class TournamentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if any(v < 0 or v != v or v in (float("inf"), float("-inf")) for _, v in weights):
            raise ValueError("metric weights must be finite and non-negative")
        object.__setattr__(self, "metric_weights", weights)
        fractions = tuple(float(v) for v in self.split_boundaries)
        if len(fractions) != 3 or any(v < 0 or v != v or v in (float("inf"), float("-inf")) for v in fractions) or abs(sum(fractions) - 1.0) > 1e-9:
            raise ValueError("split_boundaries must be three finite non-negative values summing to 1.0")
        object.__setattr__(self, "split_boundaries", fractions)
        lag = int(self.max_markov_lag)
        if lag < 1 or lag > MAX_MARKOV_LAG:
            raise ValueError(f"max_markov_lag must be in [1, {MAX_MARKOV_LAG}]")
        object.__setattr__(self, "max_markov_lag", lag)
        object.__setattr__(self, "tie_break_policy", tuple(str(v) for v in self.tie_break_policy))

    def canonical_payload(self) -> dict[str, Any]:
        return {"config_version": self.config_version, "metric_weights": list(self.metric_weights), "split_boundaries": list(self.split_boundaries), "max_markov_lag": self.max_markov_lag, "complexity_definition": self.complexity_definition, "stability_definition": self.stability_definition, "tie_break_policy": list(self.tie_break_policy), "deterministic": self.deterministic, "telemetry_schema_version": self.telemetry_schema_version}
...
class FeatureDeclaration:
    ...
    def __post_init__(self) -> None:
        ...
        if offset < 0:
            raise ValueError("feature temporal offset must be non-negative")
        object.__setattr__(self, "max_temporal_offset", offset)
        object.__setattr__(self, "derived_from", tuple(str(v) for v in self.derived_from))
...
class TemporalCausalGate:
    ...
    def __post_init__(self) -> None:
        ...
        if lag < 1 or lag > MAX_MARKOV_LAG:
            raise ValueError(f"max_lookback must be in [1, {MAX_MARKOV_LAG}]")
        object.__setattr__(self, "max_lookback", lag)
        object.__setattr__(self, "features", tuple(self.features))

    def feature_horizons(self) -> dict[str, int]:
        declarations = {f.name: f for f in self.features}
        memo: dict[str, int] = {}
        def horizon(name: str, trail: tuple[str, ...] = ()) -> int:
            ...
            value = max([f.max_temporal_offset] + [horizon(p, trail + (name,)) for p in f.derived_from])
            memo[name] = value
            return value
    ...
    def validate_features(self) -> None:
        horizons = self.feature_horizons()
        violations = {name: h for name, h in horizons.items() if h > self.max_lookback}
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"gate_version": self.gate_version, "max_lookback": self.max_lookback, "features": [{"name": f.name, "max_temporal_offset": f.max_temporal_offset, "episode_boundary_aware": f.episode_boundary_aware, "derived_from": list(f.derived_from)} for f in self.features]}
