...
def _common_panel() -> tuple[tuple[int, ...], dict[str, tuple[float, ...]], tuple[tuple[str, int, int], ...]]:
    """Build one panel and record exact segment boundaries.

    Labels are generated by the segment mechanism first. Every candidate then
    receives a prediction for those exact labels; no candidate prediction is
    copied from a different world. The XOR segment is generated from A/B states
    and A_x_B is calculated from the component probabilities.
    """
    labels: list[int] = []
    ...
    for segment in SEGMENTS:
        start = len(labels)
        if segment == "A_x_B":
            _, _, seg_labels, _ = _interaction_components()
            a, b, _, axb = _interaction_components()
            component_predictions = {
                "A_x_B": axb,
                "state": _weak(seg_labels),
                "calibrated": _weak(seg_labels),
                "delayed": _weak(seg_labels),
                "initial_momentum": _weak(seg_labels),
                "path": _weak(seg_labels),
                "resistance": _weak(seg_labels),
            }
            # Keep the component streams available to the anti-leakage test via
            # the mathematical construction, while only exposing the survivor
            # candidate A_x_B to the tournament panel.
            _ = (a, b)
        else:
