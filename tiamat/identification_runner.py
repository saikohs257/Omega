...
class IdentificationRunner:
    ...
    def axes_for(self, model_id: str) -> tuple[str, ...]:
        return self.control_axes if model_id == self.control_model_id else axes_for_model(model_id)
    ...
    def holdout_experiment(self, *, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2, probability_predictor: ProbabilityPredictor | None = None):
        from .holdout import HoldoutExperiment
        return HoldoutExperiment(runner=self, train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction, adapter=self.adapter, probability_predictor=probability_predictor)
    ...
    def evaluate_holdout(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, model_ids: Sequence[str] | None = None, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2, probability_predictor: ProbabilityPredictor | None = None, implementation_hash: str | None = None):
        return self.holdout_experiment(train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction, probability_predictor=probability_predictor).evaluate(rows, model_ids=model_ids, implementation_hash=implementation_hash, probability_predictor=probability_predictor)
    ...
    def _transition_error(self, rows: tuple[TelemetryRow, ...], axes: tuple[str, ...]) -> float:
        ...
        for before, after in zip(rows, rows[1:]):
            errors.append(self._state_error(self.transition_fn(before.to_state(), after.to_mapping()), after.to_state(), axes))
    ...
    def _trial_order(trial: CandidateTrial) -> tuple[float, float, int, str]:
        return (trial.transition_error if trial.transition_error is not None else 1.0, -trial.coverage, trial.complexity, trial.model_id)
