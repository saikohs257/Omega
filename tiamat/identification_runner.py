...
class IdentificationRunner:
    ...
    def evaluate(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, model_ids: Sequence[str] | None = None) -> TournamentReport:
        normalized = self.normalize_rows(rows); ids = tuple(model_ids or MODEL_REGISTRY.keys())
        return TournamentReport(tuple(sorted((self._evaluate_model(normalized, model_id) for model_id in ids), key=self._trial_order)))
    def holdout_experiment(self, *, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2):
        ...
        return HoldoutExperiment(runner=self, train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction, adapter=self.adapter)
    def split_rows(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2):
        return self.holdout_experiment(train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction).split_rows(rows)
    def evaluate_holdout(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, model_ids: Sequence[str] | None = None, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2):
        return self.holdout_experiment(train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction).evaluate(rows, model_ids=model_ids)
    ...
    def _evaluate_model(self, rows: tuple[TelemetryRow, ...], model_id: str) -> CandidateTrial:
        axes = self.axes_for(model_id); supported = tuple(row for row in rows if row.supports(model_id, control_axes=self.control_axes)); coverage = len(supported) / len(rows) if rows else 0.0; transition_error = self._transition_error(rows, axes) if len(rows) > 1 else None; spec = MODEL_REGISTRY.get(model_id)
        return CandidateTrial(model_id=model_id, role=spec.role if spec else "candidate", axes=axes, rows=len(rows), supported_rows=len(supported), coverage=coverage, transition_error=transition_error, complexity=len(axes), is_control=(model_id == self.control_model_id))
    def _transition_error(self, rows: tuple[TelemetryRow, ...], axes: tuple[str, ...]) -> float:
        errors = [self._state_error(self.transition_fn(before.to_state(), after.to_mapping()), after.to_state(), axes) for before, after in zip(rows, rows[1:])]
    ...
    def _state_error(self, predicted: TiamatState, observed: TiamatState, axes: tuple[str, ...]) -> float:
        ...
        for axis in axes:
            ...
            if axis in {"B", "D"}: errors.append(min(1.0, abs(float(pv) - float(ov))))
                ...
                of = float(ov); errors.append(min(1.0, abs(float(pv) - of) / max(1.0, abs(of))))
            elif axis in {"V", "Phi"}:
                pf, of = float(pv), float(ov); errors.append(1.0 if not math.isfinite(pf) or not math.isfinite(of) else min(1.0, abs(pf - of)))
            else: errors.append(0.0 if pv == ov else 1.0)
        return fmean(errors) if errors else 0.0
    @staticmethod
    def _trial_order(trial: CandidateTrial) -> tuple[float, float, int, str]: return (trial.transition_error if trial.transition_error is not None else 1.0, -trial.coverage, trial.complexity, trial.model_id)
