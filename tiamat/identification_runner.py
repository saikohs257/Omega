...
class IdentificationRunner:
    ...
    def axes_for(self, model_id: str) -> tuple[str, ...]:
        if model_id == self.control_model_id: return self.control_axes
    ...
    def holdout_experiment(self, *, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2, config: TournamentConfig | None = None, implementation_hash: str | None = None):
        from .holdout import HoldoutExperiment
        return HoldoutExperiment(runner=self, train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction, adapter=self.adapter, config=config, implementation_hash=implementation_hash or "UNBOUND")

    def split_rows(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2, config: TournamentConfig | None = None):
        return self.holdout_experiment(train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction, config=config).split_rows(rows)

    def evaluate_holdout(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, model_ids: Sequence[str] | None = None, train_fraction: float = 0.6, validation_fraction: float = 0.2, test_fraction: float = 0.2, config: TournamentConfig | None = None, implementation_hash: str | None = None):
        return self.holdout_experiment(train_fraction=train_fraction, validation_fraction=validation_fraction, test_fraction=test_fraction, config=config, implementation_hash=implementation_hash).evaluate(rows, model_ids=model_ids)
    ...
    def _evaluate_model(self, rows: tuple[TelemetryRow, ...], model_id: str) -> CandidateTrial:
        ...
        return CandidateTrial(model_id=model_id, role=role, axes=axes, rows=len(rows), supported_rows=len(supported), coverage=coverage, transition_error=transition_error, complexity=len(axes), is_control=(model_id == self.control_model_id))
    ...
    def _state_error(self, predicted: TiamatState, observed: TiamatState, axes: tuple[str, ...]) -> float:
        ...
        for axis in axes:
            ...
            if predicted_value is None or observed_value is None: errors.append(1.0); continue
            if axis in {"B", "D"}: errors.append(min(1.0, abs(float(predicted_value) - float(observed_value))))
            elif axis in {"tau_D", "tau_mode"}:
                observed_float = float(observed_value); scale = max(1.0, abs(observed_float)); errors.append(min(1.0, abs(float(predicted_value) - observed_float) / scale))
            elif axis in {"V", "Phi"}:
                predicted_float = float(predicted_value); observed_float = float(observed_value)
                errors.append(1.0 if not math.isfinite(predicted_float) or not math.isfinite(observed_float) else min(1.0, abs(predicted_float - observed_float)))
            else: errors.append(0.0 if predicted_value == observed_value else 1.0)
        return fmean(errors) if errors else 0.0

    @staticmethod
    def _trial_order(trial: CandidateTrial) -> tuple[float, float, float, int, str]:
        return (-trial.score, trial.transition_error if trial.transition_error is not None else 1.0, -trial.coverage, trial.complexity, trial.model_id)
