...
class CandidateSpec:
    ...
    def __post_init__(self) -> None:
        if not self.model_id: raise ValueError("model_id must be non-empty")
        if not self.features: raise ValueError("features must be non-empty")
        if len(set(self.features)) != len(self.features): raise ValueError("features must be unique")
        if self.complexity is not None and self.complexity < 1: raise ValueError("complexity must be positive")
    @property
    def size(self) -> int: return self.complexity if self.complexity is not None else len(self.features)
...
class ModelMetrics:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.brier <= 1.0: raise ValueError("brier must be in [0, 1]")
        if self.log_loss < 0.0: raise ValueError("log_loss cannot be negative")
        if not 0.0 <= self.stability <= 1.0: raise ValueError("stability must be in [0, 1]")
        if not 0.0 <= self.calibration_error <= 1.0: raise ValueError("calibration_error must be in [0, 1]")
        if self.complexity < 1: raise ValueError("complexity must be positive")
        if self.evaluated_n < 0: raise ValueError("evaluated_n cannot be negative")
...
def log_loss(probabilities: Sequence[float], labels: Sequence[int]) -> float:
    ...
    for p, y in zip(probabilities, labels):
        p = min(1.0 - _EPS, max(_EPS, float(p))); y = int(y)
        if y not in (0, 1): raise ValueError("labels must be binary")
        total -= y * log(p) + (1 - y) * log(1.0 - p)
    return total / len(probabilities)

def binary_auc(probabilities: Sequence[float], labels: Sequence[int]) -> float:
    _check_lengths(probabilities, labels)
    positives = sum(int(y) for y in labels); negatives = len(labels) - positives
    if positives == 0 or negatives == 0: raise ValueError("AUC requires both positive and negative labels")
    ...
    while i < len(ordered):
        ...
        rank_sum += avg_rank * sum(y for _, y in ordered[i:j]); rank += j - i; i = j
...
def calibration_error(probabilities: Sequence[float], labels: Sequence[int], *, bins: int = 10) -> float:
    ...
    for p, y in zip(probabilities, labels):
        pf = min(1.0, max(0.0, float(p))); idx = min(bins - 1, int(pf * bins)); buckets[idx].append((pf, int(y)))
    n = len(probabilities); error = 0.0
...
def consensus(probabilities: Mapping[str, float], *, tolerance: float = 0.10) -> tuple[str, float, tuple[str, ...]]:
    if not probabilities: return "UNRESOLVED", 0.0, ()
    values = tuple(float(v) for v in probabilities.values()); mean = sum(values) / len(values); spread = max(values) - min(values)
    if spread > tolerance: return "CONTESTED", mean, tuple(sorted(probabilities))
...
def _check_lengths(probabilities: Sequence[float], labels: Sequence[int]) -> None:
    if len(probabilities) != len(labels): raise ValueError("probabilities and labels must have equal length")
