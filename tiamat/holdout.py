...
class HoldoutSplit:
    train:tuple[TelemetryRow,...]; validation:tuple[TelemetryRow,...]; test:tuple[TelemetryRow,...]
    @property
...
class HoldoutEvaluation:
    split:HoldoutSplit; train:TournamentReport; validation:TournamentReport; test:Any; config:TournamentConfig; gate:TemporalCausalGate; manifest:ExperimentManifest; probability_contract:ProbabilityContract; metric_contract:MetricContract
    @property
    ...
    def to_dict(self)->dict[str,Any]: return {"version":HOLDOUT_EXPERIMENT_VERSION,"experiment_id":self.experiment_id,"manifest":self.manifest.to_dict(),"config":self.config.to_dict(),"causal_gate":self.gate.to_dict(),"probability_contract":self.probability_contract.to_dict(),"metric_contract":self.metric_contract.to_dict(),"locked_model_id":self.locked_model_id,"test_selected":None if self.test is None else {"model_id":self.test.model_id,"coverage":self.test.coverage,"transition_error":self.test.transition_error,"score":self.test.score},"split":self.split.to_dict()}
@dataclass(frozen=True,slots=True)
class HoldoutExperiment:
    runner:IdentificationRunner=field(default_factory=IdentificationRunner); train_fraction:float=0.6; validation_fraction:float=0.2; test_fraction:float=0.2; adapter:TelemetryAdapter=field(default_factory=TelemetryAdapter); config:TournamentConfig|None=None; gate:TemporalCausalGate=field(default_factory=TemporalCausalGate); implementation_hash:str="UNBOUND"; probability_contract:ProbabilityContract=field(default_factory=lambda:ProbabilityContract(STATE_SPACE)); probability_predictor:ProbabilityPredictor|None=None; label_provenance:LabelProvenance|None=None; metric_contract:MetricContract|None=None
    def __post_init__(self)->None:
        ...
        if self.metric_contract is None: object.__setattr__(self,"metric_contract",MetricContract(self.probability_contract))
        elif self.metric_contract.probability_contract!=self.probability_contract: raise ValueError("metric_contract probability contract must match experiment probability contract")
    ...
    def split_rows(self,rows:Sequence[Mapping[str,Any]|TelemetryRow],*,model_id:str="M3")->HoldoutSplit:
        self.gate.validate_rows(rows); normalized=self._order_rows(self.normalize_rows(rows,model_id=model_id)); a,b,c=self._allocate_counts(len(normalized),self.config.split_boundaries); return HoldoutSplit(tuple(normalized[:a]),tuple(normalized[a:a+b]),tuple(normalized[a+b:a+b+c]))
    def _feature_provenance_hash(self,model_id:str)->str:
        spec=MODEL_REGISTRY[model_id]; axes=spec.state if isinstance(spec.state,tuple) else (); gate=replace(self.gate,features=tuple(FeatureDeclaration(name=axis) for axis in axes)); gate.validate_features(); return provenance_fingerprint(gate.to_dict())
    def evaluate(self,rows:Sequence[Mapping[str,Any]|TelemetryRow],*,model_ids:Sequence[str]|None=None,implementation_hash:str|None=None,probability_predictor:ProbabilityPredictor|None=None)->HoldoutEvaluation:
        ...
        if predictor is None: raise ValueError("probability_predictor is required before holdout evaluation")
        if self.label_provenance is None: raise ValueError("label_provenance is required before holdout evaluation; declare observed, proxy, or model_generated provenance")
        split=self.split_rows(rows); ids=tuple(model_ids) if model_ids is not None else None; train_report=self.runner.evaluate(split.train,model_ids=ids); validation_report=self.runner.evaluate(split.validation,model_ids=ids); selected=validation_report.winner.model_id if validation_report.winner else None
        if selected is not None:
            for row in tuple(self.runner.normalize_rows(split.test,model_id=selected)): validate_probability_output(row,predictor,self.probability_contract)
        locked=None if selected is None else LockedModelEvaluator(selected,self.runner,predictor,self.probability_contract); test_result=None if locked is None else locked.evaluate(split.test)
        corpus_hash=corpus_fingerprint([r.to_mapping() for r in (*split.train,*split.validation,*split.test)]); label=self.label_provenance
        if label.label_corpus_hash!=corpus_hash: raise ValueError("label_provenance label_corpus_hash does not match holdout corpus")
        label.validate(max_declared_future_steps=self.config.max_markov_lag); label_hash=provenance_fingerprint(label.to_dict()); manifest_model_id=selected or (ids[0] if ids else "M3"); feature_hash=self._feature_provenance_hash(manifest_model_id); probability_hash=provenance_fingerprint(self.probability_contract.to_dict()); metric_hash=provenance_fingerprint(self.metric_contract.to_dict()); impl_hash=implementation_hash or self.implementation_hash
