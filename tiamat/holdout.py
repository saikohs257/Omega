...
from .metric_contract import LabelProvenance, ProbabilityContract, ProbabilityPredictor, STATE_SPACE, validate_probability_output
...
class HoldoutExperiment:
    ...
    def evaluate(self, rows: Sequence[Mapping[str,Any]|TelemetryRow], *, model_ids: Sequence[str]|None=None, implementation_hash: str|None=None, probability_predictor: ProbabilityPredictor|None=None) -> HoldoutEvaluation:
        ...
        split=self.split_rows(rows); ids=tuple(model_ids) if model_ids is not None else None
        train_report=self.runner.evaluate(split.train,model_ids=ids); validation_report=self.runner.evaluate(split.validation,model_ids=ids); selected=validation_report.winner.model_id if validation_report.winner else None
        ...
        return HoldoutEvaluation(split,train_report,validation_report,test_result,self.config,self.gate,manifest,self.probability_contract)
    @staticmethod
