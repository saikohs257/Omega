...
class HoldoutExperiment:
    ...
    def __post_init__(self)->None:
        ...
        if self.label_provenance is None: object.__setattr__(self,"label_provenance",LabelProvenance("observed","mode-v1","observed controller mode",1.0,("mode",),"UNBOUND",0,False))
    ...
    def evaluate(self,rows:Sequence[Mapping[str,Any]|TelemetryRow],*,model_ids:Sequence[str]|None=None,implementation_hash:str|None=None,probability_predictor:ProbabilityPredictor|None=None)->HoldoutEvaluation:
        ...
        label.validate(max_declared_future_steps=self.config.max_markov_lag); label_hash=provenance_fingerprint(label.to_dict()); feature_hash=self._feature_provenance_hash(manifest_model_id); probability_hash=provenance_fingerprint(self.probability_contract.to_dict()); metric_hash=provenance_fingerprint(self.metric_contract.to_dict()); impl_hash=implementation_hash or self.implementation_hash
