...
class HoldoutEvaluation:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "test_selected": None if self.test is None else {
                "model_id": self.test.model_id,
                "coverage": self.test.coverage,
                "transition_error": self.test.transition_error,
...
class HoldoutExperiment:
    ...
    probability_contract: ProbabilityContract = field(
        default_factory=lambda: ProbabilityContract(("Q", "P", "E", "C", "H", "R", "Rf"))
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.label_provenance is None:
            object.__setattr__(
                self,
                "label_provenance",
                LabelProvenance("observed", "mode-v1", "observed controller mode", 1.0, ("mode",), "UNBOUND", 0, False),
            )
    ...
    def _feature_provenance_hash(self, model_id: str) -> str:
        spec = self.runner.registry[model_id]
        features = tuple(FeatureDeclaration(name=axis) for axis in (spec.state if isinstance(spec.state, tuple) else ()))
