...
from .metric_contract import LabelProvenance, ProbabilityContract, ProbabilityPredictor, STATE_SPACE
...
class HoldoutExperiment:
    ...
    def evaluate(self, rows: Sequence[Mapping[str, Any] | TelemetryRow], *, model_ids: Sequence[str] | None = None, implementation_hash: str | None = None, probability_predictor: ProbabilityPredictor | None = None) -> HoldoutEvaluation:
        ...
        train_report = self.runner.evaluate(split.train, model_ids=ids); validation_report = self.runner.evaluate(split.validation, model_ids=ids)
        ...
        manifest_model_id = selected or (ids[0] if ids else "M3"); label = self.label_provenance
